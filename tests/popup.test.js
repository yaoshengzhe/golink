...
describe('Popup', () => {
  ...
  describe('Initial State', () => {
    test('should show empty state when no mappings exist', async () => {
      ...
      const emptyStateVisible = await page.evaluate(() => {
        return !document.getElementById('emptyState').classList.contains('hidden');
      });
    ...
    test('should show loading state initially', async () => {
      ...
      const loadingVisible = await page.evaluate(() => {
        return !document.getElementById('loadingSpinner').classList.contains('hidden');
      });
  ...
  describe('Mappings Display', () => {
    beforeEach(async () => {
      ...
      const testMappings = [
        ...
        { shortName: 'github', url: 'https://github.com/company/repo', description: '' }
      ];
    ...
    test('should display mapping details correctly', async () => {
      const firstLinkShortName = await page.$eval('.link-item:first-child .link-short-name', el => el.textContent);
      const firstLinkUrl = await page.$eval('.link-item:first-child .link-url', el => el.textContent);
  ...
  describe('Search Functionality', () => {
    beforeEach(async () => {
      // Create test mappings
      const testMappings = [
        { shortName: 'docs', url: 'https://docs.example.com', description: 'Documentation site' },
        { shortName: 'jira', url: 'https://jira.example.com', description: 'Issue tracker' },
        { shortName: 'confluence', url: 'https://wiki.example.com', description: 'Wiki and docs' }
      ];
    ...
    test('should filter by URL', async () => {
      ...
      const shortName = await page.$eval('.link-item .link-short-name', el => el.textContent);
  ...
  describe('Edit Modal', () => {
    ...
    test('should save changes successfully', async () => {
      ...
      const description = await page.$eval('.link-description', el => el.textContent);
