...
describe('Omnibox Unit Tests', () => {
  ...
  describe('Omnibox Input Handling', () => {
    test('should handle valid go-link input', async () => {
      ...
      expect(chrome.tabs.update).toHaveBeenCalledWith({ url: 'https://docs.example.com' });
    });
    ...
    test('should handle new tab disposition', async () => {
      ...
      expect(chrome.tabs.create).toHaveBeenCalledWith({ url: 'https://docs.example.com' });
    });

    test('should redirect to create page for non-existent mapping', async () => {
      testHelpers.mockChromeStorageGet({});
      chrome.runtime.getURL.mockReturnValue('chrome-extension://test-id/create.html');
  ...
  describe('Omnibox Suggestions', () => {
    test('should provide suggestions based on input', async () => {
      const storageData = {
        'golink_docs': testHelpers.createMockMapping('docs', 'https://docs.example.com', 'Documentation'),
        'golink_dashboard': testHelpers.createMockMapping('dashboard', 'https://dashboard.example.com', 'Main dashboard'),
        'golink_dev-tools': testHelpers.createMockMapping('dev-tools', 'https://tools.example.com', 'Development tools')
      };
      ...
      expect(mockSuggest).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({
            content: 'docs',
            description: expect.stringContaining('docs → https://docs.example.com')
          })
        ])
      );
    });
    ...
    test('should provide suggestions based on description', async () => {
      const storageData = {
        'golink_tools': testHelpers.createMockMapping('tools', 'https://tools.example.com', 'Development tools'),
        'golink_dashboard': testHelpers.createMockMapping('dashboard', 'https://dashboard.example.com', 'Main dashboard')
      };
    ...
    test('should limit suggestions to 5 items', async () => {
      const storageData = {};
      for (let i = 0; i < 10; i++) {
        storageData[`golink_link${i}`] = testHelpers.createMockMapping(`link${i}`, `https://example${i}.com`);
      }
