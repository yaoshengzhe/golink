...
describe('GoLink Management', () => {
    beforeEach(() => {
        ...
        mockExtensionAPI.runtime.sendMessage.mockImplementation((message, callback) => {
            const { action, ...data } = message;
            
            if (mockBackgroundHandlers[action]) {
                mockBackgroundHandlers[action](data).then(result => {
                    callback(result);
                }).catch(error => {
    ...
    describe('Link Retrieval', () => {
        ...
        test('should handle storage errors gracefully', async () => {
            // Mock storage error
            mockExtensionAPI.storage.local.get.mockImplementation((keys, callback) => {
    ...
    describe('Storage Persistence', () => {
        ...
        test('should handle large numbers of links', async () => {
            ...
            const promises = [];
            for (let i = 0; i < 100; i++) {
                promises.push(mockBackgroundHandlers.saveMapping({
                    shortName: `link${i}`,
