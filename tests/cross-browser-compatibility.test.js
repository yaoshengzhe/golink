...
describe('Cross-browser API Compatibility', () => {
    ...
    describe('Message Sending Compatibility', () => {
        ...
        test('should work with Chrome API', async () => {
            ...
            mockApis.chrome.runtime.sendMessage.mockImplementation((message, callback) => {
        ...
        test('should work with Safari API', async () => {
            ...
            mockApis.safari.runtime.sendMessage.mockImplementation((message, callback) => {
        ...
        test('should work with Firefox API', async () => {
            ...
            mockApis.firefox.runtime.sendMessage.mockImplementation((message, callback) => {
    ...
    describe('Safari-specific Compatibility Issues', () => {
        ...
        test('should handle Safari runtime errors gracefully', async () => {
            ...
            mockApis.safari.runtime.lastError = { message: 'Safari extension not loaded' };
            mockApis.safari.runtime.sendMessage.mockImplementation((message, callback) => {
                callback(null);
            });

            await expect(sendMessage({ action: 'test' })).rejects.toThrow('Safari extension not loaded');
        });
    });
    ...
    describe('Storage API Compatibility', () => {
        test('should work with different storage APIs', async () => {
            ...
            const chromeResult = await testStorageCompatibility(mockApis.chrome, 'chrome');
            const safariResult = await testStorageCompatibility(mockApis.safari, 'safari');
            const firefoxResult = await testStorageCompatibility(mockApis.firefox, 'firefox');
    ...
    describe('Console Logging Compatibility', () => {
        test('should log appropriate browser detection messages', () => {
            ...
            window.browser = mockApis.safari;
            const extensionAPI = window.browser || window.chrome;
            const browserType = extensionAPI === window.browser ? 'browser' : 'chrome';
    ...
    describe('Error Handling Compatibility', () => {
        test('should handle different error formats across browsers', async () => {
            ...
            const chromeError = await testErrorHandling(mockApis.chrome, 'Chrome error');
            const safariError = await testErrorHandling(mockApis.safari, 'Safari error');
            const firefoxError = await testErrorHandling(mockApis.firefox, 'Firefox error');
