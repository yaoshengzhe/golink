      ...
      document
        ...
        .addEventListener('submit', async function (e) {
          ...
          if (!shortName || !targetUrl) {
            window.showMessage(
              'Please fill in both short name and target URL',
              'error'
            );
            return;
          }
          ...
          try {
            ...
            if (response.error) {
              ...
              window.showMessage(
                `✅ Created! Now try typing: http://go/${shortName}`,
                'success'
              );
  ...
  describe('Links Management UI', () => {
    const setupLinksManagement = () => {
      ...
      window.loadLinks = async function () {
        ...
        try {
          const mappings = await window.sendMessage({
            action: 'getAllMappings',
          });

          if (!mappings || Object.keys(mappings).length === 0) {
            linksList.innerHTML =
              '<div class="empty-state">No GoLinks created yet.</div>';
            return;
          }

          const linksHtml = Object.entries(mappings)
            .map(([shortName, mapping]) =>
              window.createLinkHtml(shortName, mapping)
            )
            .join('');
          ...
          linksList.innerHTML =
            '<div class="empty-state error">Error loading links: ' +
            error.message +
            '</div>';
        }
      };
      ...
      window.deleteLink = async function (shortName) {
        try {
          ...
          if (response.error) {
            window.showMessage(
              'Error deleting link: ' + response.error,
              'error'
            );
          } else {
