...
describe('Create Page', () => {
  ...
  describe('Form Submission', () => {
    ...
    test('should handle duplicate short names', async () => {
      ...
      await page.goto(`chrome-extension://${extensionId}/create.html?shortName=duplicate`);
      
      // Should show existing mapping section
      await page.waitForSelector('#existingMapping:not(.hidden)', { timeout: 2000 });
      
      const existingUrl = await page.$eval('#existingUrl', el => el.textContent);
  ...
  describe('Pre-filled Short Name', () => {
    test('should pre-fill short name from URL parameter', async () => {
      await page.goto(`chrome-extension://${extensionId}/create.html?shortName=prefilled`);
    ...
    test('should focus on URL field when short name is pre-filled', async () => {
      await page.goto(`chrome-extension://${extensionId}/create.html?shortName=test`);
  ...
  describe('Real-time Validation', () => {
    test('should show validation error for invalid short name in real-time', async () => {
      ...
      expect(customValidity).toContain('Only letters, numbers, hyphens, and underscores allowed');
    });
