...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        if (!isValidShortName(shortName)) {
            showMessage('Short name can only contain letters, numbers, hyphens, and underscores', 'error');
            return;
        }
        ...
        try {
            ...
            if (response.error) {
                ...
                showMessage(`GoLink created successfully! You can now use go/${shortName}`, 'success');
...
async function checkExistingMapping(shortName) {
    try {
        ...
        if (response && response.url) {
            ...
            document.getElementById('existingDescription').textContent = response.description || 'No description';
